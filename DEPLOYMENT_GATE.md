...
- [ ] Apply and verify migrations `0001`–`0015` with `wrangler d1 migrations apply` after a tested backup/rollback plan. `0007_order_number_index.sql` backs the reconciliation cron's `order_number` lookup; skipping it degrades reconciliation, it does not fail loudly. `0008_payment_method.sql` adds wallet payment-method tracking used by the Apple Pay/Google Pay reconciliation split. `0009`–`0013` add Icelandic invoicing (VAT/kennitala, invoice numbering, credit notes, 7-year audit-hash retention) and incident tracking. `0014_shipping_cost.sql` adds `orders.shipping_incl_vat`, required by `assertPricingIntegrity` in `src/lib/payment-integrity.ts`. `0015_terms_acceptance.sql` adds `orders.terms_accepted_at`/`terms_version`, required by the checkout insert in `src/lib/db.ts` — omitting either `0014` or `0015` breaks checkout and invoicing, not just degrades them.
