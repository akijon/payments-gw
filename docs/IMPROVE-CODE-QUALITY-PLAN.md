...
- **Current Status:** Local release checks pass (`npm test`, `typecheck`, `lint`, `format:check` green with 113 tests across 19 files). Production deployment blocked by external environment & key configuration items (`DEPLOYMENT_GATE.md`). One internal item is open and ledgered: the circuit-breaker trial is not single-flight (`docs/TECH-DEBT.md`).
...
| ---------------------------------- | ---------------------------- | --------------------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1 — Build the safety net           | `working-with-legacy-code`   | done                                    | `docs/TESTING.md` + `docs/TECH-DEBT.md` (GATE) | Safety net map complete; characterization tests added for `verifone.ts` token refresh, `webhook.ts` concurrent delivery, `reconcile.ts`'s no-pagination assumption. 2026-08-01: `return_url` origin resolution pinned; breaker trial admission logged as unpinned. |
...
| 7 — Make it survive production     | `release-it`                 | done                                    | `docs/RELIABILITY.md`                          | `src/lib/circuit-breaker.ts` wraps Verifone/Landsbankinn calls (in-memory, per-isolate, best-effort). Deep `/health?deep=1` pings D1 + KV. 2026-08-01: audit table corrected against the code and `## Open Reliability Items` added.                               |
...
| 2026-08-01 | Phase 7 | Checkout creation stays unretried (fail fast)                          | Verifone's `x-vfi-api-idempotencykey` is documented for the eCommerce API; the Checkout API's `POST /v2/checkout` documents no idempotency header and no replay behavior. Retrying could open a second live payment session. Four vendor questions recorded in `SANDBOX_E2E_GATE.md`. |
| 2026-08-01 | Phase 7 | Webhooks stay on the Worker origin, returns use `PUBLIC_API_URL`       | Detached-JWS verification needs the byte-exact body, so webhooks must not traverse the storefront proxy; browser returns must land on the public origin the storefront serves.                                                                                                        |
| 2026-08-01 | Phase 7 | Breaker single-flight trial deferred, not fixed                        | Low risk (per-isolate blast radius), and a resilience-path behavior change has no production traffic to validate against before the deploy gate. Ledgered in `docs/TECH-DEBT.md` and flagged unpinned in `docs/TESTING.md`.                                                           |
...
- [x] 2026-08-01 doc-truth pass: `docs/RELIABILITY.md` audit table corrected against the code (timeouts, absent retries, JWKS TTL/fallback), migration `0007` propagated to all gate docs, `PUBLIC_API_URL` pinned by test and added to the Safety Net Map
...
- [ ] Get the four checkout-idempotency questions in `SANDBOX_E2E_GATE.md` answered by Verifone before any retry logic is written
- [ ] Verify `GET <PUBLIC_API_URL>/api/return` reaches this Worker (`DEPLOYMENT_GATE.md`) — external routing, blocks sandbox E2E
- [ ] Breaker single-flight trial — ledgered in `docs/TECH-DEBT.md`, low priority; pin the current admission behavior first if it is ever fixed

Every phase is `done` except Phase 8 (`deferred`). Remaining work is external — see `DEPLOYMENT_GATE.md` — plus the low-priority breaker item carried in the Debt Ledger.
