...
- [ ] `reconcile.ts` / `landsbankinn.ts`: **Known, deliberately unresolved.** `landsbankinn.ts` has no pagination handling at all today, and the Acquiring API's real pagination contract (if any) is not documented anywhere in this repo. `test/reconcile.test.ts` ("processes a large settlement transaction batch...") pins the *current* assumption — a single injected array is the complete result set — rather than guessing at unverified vendor pagination semantics (OData `top`/`skip`, a cursor, etc.). Revisit once real Landsbankinn API docs are available; see `SANDBOX_E2E_GATE.md`.
